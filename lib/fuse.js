	...
	const linkIn = (roboRoot, invertedLinkMap) => {
		...
		Reflect.ownKeys(invertedLinkMap).forEach(insideOf => {
			Reflect.ownKeys(invertedLinkMap[insideOf]).forEach(linkPackage => {
				...
				// Also link subs (now that npm makes node_modules a flat structure)
				if (typeof invertedLinkMap[linkPackage] === 'object') {
					Reflect.ownKeys(invertedLinkMap[linkPackage]).forEach(subLinkPkg => {
						const struct = {
							dependency: subLinkPkg,
							linkInside: insideOf
						};
